...
初始条件为\(z_0=[x_0,y_0,v_{x0},v_{y0}]=[1.0, 0.0, 0.0, 0.5]\)，势场强度\(\lambda=1.0\)。左图：粒子在x-y平面的运动轨迹，呈现无周期性、遍历性的非闭合不规则缠绕结构，红点标记粒子初始位置；中图：50个积分时间单位内坐标变量\(x(t)\)（蓝线）与\(y(t)\)（橙线）的时间序列；右图：哈密顿量守恒验证，全积分时段内最大绝对能量漂移\(\Delta E_{max}=3.60\times10^{-12}\)，验证了所采用四阶龙格-库塔积分方案的高数值精度。
